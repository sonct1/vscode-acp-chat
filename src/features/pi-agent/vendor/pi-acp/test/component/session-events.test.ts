...
test('PiAcpSession: prompt resolves end_turn on agent_end and emits usage_update', async () => {
  const conn = new FakeAgentSideConnection()
  const proc = new FakePiRpcProcess()
  proc.sessionStats = {
  ...
  const usageUpdate = conn.updates.find(update => update.update.sessionUpdate === 'usage_update')
