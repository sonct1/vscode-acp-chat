...
- New Chat creates and activates a local draft immediately, starts the selected agent runtime in the background, and does not create an ACP session/history entry until the first prompt or explicit session action needs one;
...
- switching sessions shows a revision-safe loading overlay below the multi-session header, temporarily locks transcript/composer interactions during cold snapshot replay, and ignores stale snapshots from older activations;
