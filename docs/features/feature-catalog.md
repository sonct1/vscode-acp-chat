...
`opencode`, `claude-code`, `codex`, `gemini`, `goose`, `amp`, `aider`, `augment`, `kimi`, `mistral-vibe`, `openhands`, `qwen-code`, `kiro`, `cursor`, `codebuddy`, `pi`, opt-in `antigravity`, and opt-in experimental `swarm`.
...
- New Chat creates and activates a local draft immediately, reveals/focuses the chat view when launched from commands or Session Manager, restores the matching draft snapshot, unlocks the composer, then focuses the rich composer input so typing can start immediately; it starts the selected agent runtime in the background and does not create an ACP session/history entry until the first prompt or explicit session action needs one;
...
- the chat webview shows only the active session switch affordance and active title/status; the separate Primary Sidebar manager view shows aggregate running, permission, input, and open counts plus running, idle, draft, `awaiting_permission`, `awaiting_input`, error, and closed sessions;
...
- chat webview → host: `feature.acp-elicitation.respond`; `feature.multi-session.ready`, `new`, `activate`, `stop`, `close`, `openManagerPanel`, `quickSwitch`, `resync`, `reviewPermission`, `reviewInput`, `permission.respond`;
- host → chat webview: `feature.acp-elicitation.show`, `feature.acp-elicitation.validation`; `feature.multi-session.chatState`, `feature.multi-session.snapshot`, `feature.multi-session.delta`, guarded one-shot `feature.multi-session.focusInput` for New Chat activations;
- manager view → host: `feature.multi-session.managerReady`, `managerResync`, `new`, `activate`, `stop`, `close`, `reviewPermission`, `reviewInput`;
