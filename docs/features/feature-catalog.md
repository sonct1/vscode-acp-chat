...
- switching/opening existing sessions, loading history, passive resync/snapshot refreshes, permission review, input review, and Clear Chat do not steal composer focus; background input requests do not activate themselves, while `Review input` activates the owning session and focuses the chat view; switching sessions shows a revision-safe loading overlay below the multi-session header, temporarily locks transcript/composer interactions during cold snapshot replay, and ignores stale snapshots/focus intents from older activations;
