...
| Experimental Swarm agent infrastructure                | Adds an opt-in bundled ACP root orchestrator that runs user-defined role/workflow DAGs through dedicated worker ACP sessions with capability policy, locks, evidence, and live progress.                                                                                                              | `vscode-acp-chat.swarmAgent.enabled`, `.vscode/acp-swarm/`, agent selector `Swarm (Experimental)`.            | [`src/features/swarm-agent/`](../../src/features/swarm-agent/), [`examples/acp-swarm/`](../../examples/acp-swarm/)                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Tool visibility, terminal integration, and permissions | Shows agent tool activity, terminal output, permission prompts, and command results; permission prompts are live queue-owned UI state, not durable transcript history.                                                                                                                                 | ACP client fs/terminal/permission callbacks and webview tool blocks.                                          | [`src/acp/client.ts`](../../src/acp/client.ts), [`src/acp/terminal-handler.ts`](../../src/acp/terminal-handler.ts), [`src/views/webview/block/tool-block.ts`](../../src/views/webview/block/tool-block.ts), [`src/features/permission-ui/`](../../src/features/permission-ui/), [`src/views/webview/widget/permission-dialog.ts`](../../src/views/webview/widget/permission-dialog.ts)                                                                                                                                                                                        |
| Structured ACP elicitation forms                       | Lets compatible ACP agents pause a turn and request validated text, choices, numbers, booleans, dates, or other structured form input without misusing permission dialogs.                                                                                                                            | Standard ACP `elicitation/create` form requests; form panel above composer; Session Manager `Review input`.   | [`src/acp/client.ts`](../../src/acp/client.ts), [`src/features/acp-elicitation/`](../../src/features/acp-elicitation/), [`src/features/multi-session/`](../../src/features/multi-session/)                                                                                                                                                                                                                                                                                                                                                                                   |
...
| Transcript navigation helpers                          | Speeds up prompt and response navigation with keyboard recall, assistant-turn controls, and contextual user-prompt tip arrows.                                                                                                                                                                        | Up/Down in prompt, assistant previous/next header buttons, tip previous/next buttons.                         | [`src/features/prompt-history-navigation/`](../../src/features/prompt-history-navigation/), [`src/features/assistant-turn-navigation/`](../../src/features/assistant-turn-navigation/), [`src/features/latest-user-prompt-tip/`](../../src/features/latest-user-prompt-tip/)                                                                                                                                                                                                                                                                                                 |
...
Grok Build behavior: the built-in `grok-build` entry launches `grok --no-auto-update agent stdio` and is available when `grok` resolves from the Extension Host environment. Users authenticate out-of-band with `grok login` or provide `XAI_API_KEY`; the extension does not select ACP auth methods or manage Grok credentials. Grok's advertised `loadSession` capability is used, while absent `sessionCapabilities.list` means history listing falls back to extension-local session metadata instead of calling `session/list`. The ACP client accepts old-format `models` returned by compatible agents, with `configOptions` retaining precedence, and falls back to legacy `session/set_model` when the modern model config method is unavailable. A custom agent with `id: "grok-build"` overrides the built-in entry.
...
Swarm behavior: when `vscode-acp-chat.swarmAgent.enabled` is `false`, the bundled Swarm entry is omitted from the built-in catalog, but a custom agent with `id: "swarm"` remains valid. When enabled, the bundled entry launches `process.execPath dist/swarm-acp/index.mjs` with `ELECTRON_RUN_AS_NODE=1` and materializes a runtime JSON config before connecting. Swarm reads `.vscode/acp-swarm/swarm.config.json`, `roles/*.json`, and `workflows/*.json`; `rootRole` is required and must reference a real role whose `agentId` resolves. Roles and workflow steps are arbitrary ids, not a fixed planner/implementer/reviewer/proof sequence. For each user prompt, the persistent Root ACP session makes a hidden strict JSON routing decision: answer directly or run exactly one configured workflow. `defaultWorkflow` is only a backward-compatible routing tie-break hint and is never auto-executed after malformed routing. Each workflow step runs its configured ACP agent in a dedicated worker process/session. Capability policy is enforced in the adapter for file read/write, terminal access, permission context, path write locks, named locks, and the `test_runner` lock. Live progress appears as `swarm_run`, `swarm_step`, `swarm_worker`, `swarm_lock`, and `swarm_evidence` tool cards. Limitations: no Extension Host restart resume, no automatic commits, no visual workflow editor, and worker compatibility depends on each configured ACP agent.
...
When the active transcript is more than 100 px away from the bottom, the composer shows a single-line `Tip:` preview of the user prompt associated with the conversation turn currently being read, plus right-aligned previous/next icon buttons. The feature uses a reading anchor in the upper quarter of the message viewport: scrolling upward across turn boundaries changes to older prompts, and scrolling downward changes to newer prompts. The buttons smooth-scroll to the previous older or next newer textual user prompt, skip non-text turns, disable at the first/last textual prompt, and do not load content into the composer or share state with prompt-history recall. Assistant text, thought, or tool streaming alone does not change the selected prompt while the user's reading position is unchanged. The tip hides near the bottom, after chat clear, or when the active turn has no textual prompt. Live turns, duplicate prompt text, loaded history, resized viewports, and multi-session snapshot replay all derive the prompt from the active transcript DOM, active index and restored scroll position. The truncated row is keyboard-focusable and expands to expose the full prompt; focusing the arrow buttons keeps the preview ellipsized.
...
- authoritative owner-scoped permission state that replaces previous pending UI on ready, session switch, resync, or snapshot replay, so answered permissions do not reappear as transcript history;
- allow and reject buttons return ACP `selected` with the agent-provided option id, while `cancelled` is reserved for actual cancellation such as Stop, timeout, disconnect, runtime replacement, or disposal;
...
### Structured ACP elicitation forms

The extension conditionally advertises the unstable ACP `elicitation.form` client capability when the generic host handler is bound. Any compatible agent can then send standard `elicitation/create` requests; the implementation does not branch on agent id. URL-mode elicitation and vendor-specific custom question methods are not advertised or handled by this feature.

Supported form values include plain and formatted strings (`email`, `uri`, `date`, `date-time`), titled or untitled single-select choices, string multi-select choices, finite numbers, safe integers, and booleans. Agent schemas are treated as untrusted: unsupported/custom fields, regular-expression patterns, invalid defaults or ranges, duplicate options, and oversized forms fail closed with `cancel`. Answers are validated again in the Extension Host before they are returned as `accept`; `decline`, user `cancel`, and agent JSON-RPC cancellation remain distinct outcomes. Answer values are kept only in memory and are not written to transcript, webview persisted state, or extension state.

The accessible form panel is rendered immediately above the composer, outside the transcript. Only the oldest pending request is shown, with a FIFO count; composer drafts, Stop controls, message queue state, and `isGenerating` remain independent. Legacy mode uses one `legacy` owner. Multi-session mode uses `localSessionId` owners, so active-session forms render immediately while background forms stay queued, set status `awaiting_input`, add `N input` badges/aggregate counts, and expose `Review input` without stealing focus. Stop, close, runtime disposal, connection loss, or extension disposal cancel the relevant pending requests.
...
- New Chat creates and activates a local draft immediately, reveals/focuses the chat view when launched from commands or Session Manager, restores the matching draft snapshot, unlocks the composer, then focuses the rich composer input so typing can start immediately; it may start the selected agent runtime connection in the background, but remains lazy and does not create or resume a real ACP session until the first send or session-affecting action;
- on reload, the active workspace session binding (`agentId`, ACP `sessionId`, `cwd`, optional title) is restored when valid; the first `feature.multi-session.ready` starts only that original active agent and either loads the saved ACP session with history replay semantics or creates and persists a replacement session when loading is unsupported or the saved session is narrowly reported missing; if there is no saved binding, that first ready creates a new ACP session immediately;
- each session has independent transcript, draft, scroll state, ACP runtime, metadata, live permission queue, pending elicitation forms, and status; permissions are not stored as durable transcript events and background-session permissions stay isolated until that owner is active;
...
- the chat webview shows only the active session switch affordance and active title/status; the separate Primary Sidebar manager view shows aggregate running, permission, input, and open counts plus running, idle, draft, `awaiting_permission`, `awaiting_input`, error, and closed sessions; error rows keep the error text visible and expose a Retry action that clears the error, restarts only that local runtime, and reloads an existing ACP id before creating a replacement when needed;
...
- switching/opening existing sessions, loading history, passive resync/snapshot refreshes, and Clear Chat do not steal composer focus (session switch explicitly requests input focus through the same guarded handshake as New Chat); background input requests do not activate themselves, while `Review input` activates the owning session and focuses the chat view; switching sessions shows a revision-safe loading overlay below the multi-session header, temporarily locks transcript/composer interactions during cold snapshot replay, and ignores stale snapshots/focus intents from older activations;
...
- chat webview → host: `feature.acp-elicitation.respond`; `feature.multi-session.ready`, `new`, `activate`, `stop`, `close`, `retry`, `openManagerPanel`, `quickSwitch`, `resync`, `reviewPermission`, `reviewInput`, `permission.respond`, plus `focusInputArmed` / `focusInputAck` for verified composer focus;
- host → chat webview: `feature.acp-elicitation.show`, `feature.acp-elicitation.validation`; `feature.multi-session.chatState`, `feature.multi-session.snapshot`, `feature.multi-session.delta`, and guarded `focusInput` / `focusInputCommit` messages for New Chat activations;
- manager view → host: `feature.multi-session.managerReady`, `managerResync`, `new`, `activate`, `stop`, `close`, `retry`, `reviewPermission`, `reviewInput`;
...
Loaded multi-session history is collected without transcript-content deltas and published once as a finalized snapshot. Text/thought Markdown is rendered once per finalized block; historical tools skip current-workspace diff reconstruction, keep agent-provided content, and defer cached details/diff rendering until expansion. The bundled Pi adapter defaults `vscode-acp-chat.pi.historyLoadMode` to `full`, replaying the active-path transcript from Pi JSONL session files during `session/load`; `compacted` preserves the previous behavior of replaying Pi's compacted `get_messages` RPC response. Pi session listing uses a size/mtime-validated JSONL metadata index, a short-lived warm discovery snapshot that can avoid a repeated recursive walk/stat, and reusable cursor snapshots; a listed session is resolved through a persisted direct file mapping before any fallback discovery. Pi full-history replay coalesces adjacent historical user/assistant text fragments before emitting ACP updates while keeping tool result boundaries and final tool status/content. Full replay is display-only and does not resend the raw transcript to the model when continuing a chat.
...
- assistant-turn navigation: previous/next icon controls in the multi-session header, or the message-container fallback when the header is absent, appear as soon as one completed assistant response with text exists and remain enabled. Tool-only/textless responses are excluded. After manual scrolling, `Next` selects the nearest answer-text target below the reading anchor while `Previous` selects the nearest target above it; repeated button presses then continue sequentially from the selected response. Multi-session activation/replay starts a versioned transcript-surface replacement lifecycle that immediately hides and invalidates the navigator, then rebuilds it only after transcript replay and scroll restoration, preventing stale button/focus anchors or detached DOM entries from a previously opened session. At the first or last response, the corresponding control re-scrolls that same response instead of wrapping. The feature keeps an `Assistant n / total` counter for accessibility/state and anchors navigation on the first non-empty `.block-text`; navigation does not focus or highlight the response.
...
| `vscode-acp-chat.swarmAgent.enabled`                     | `false`                | Enables the bundled experimental Swarm root orchestrator.                                      |
| `vscode-acp-chat.swarmAgent.configDirectory`             | `.vscode/acp-swarm`    | Directory containing Swarm `swarm.config.json`, roles, and workflows.                          |
| `vscode-acp-chat.swarmAgent.defaultWorkflow`             | `default`              | Default workflow id used when Swarm materializes runtime config.                               |
| `vscode-acp-chat.swarmAgent.maxWorkers`                  | `4`                    | Caps concurrent Swarm worker sessions.                                                         |
| `vscode-acp-chat.swarmAgent.requireApprovalBeforeWrites` | `true`                 | Requires permission before Swarm forwards write-capable worker actions.                        |
| `vscode-acp-chat.swarmAgent.testLockPatterns`            | test command fragments | Terminal command fragments that acquire the Swarm `test_runner` lock.                          |
...
- ACP agent capabilities determine whether history, MCP transports, document sync, modes, models, commands, generic options, and standard form elicitation are available.
