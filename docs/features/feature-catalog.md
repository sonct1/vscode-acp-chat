...
- switching/opening existing sessions, loading history, passive resync/snapshot refreshes, permission review, and Clear Chat do not steal composer focus; switching sessions shows a revision-safe loading overlay below the multi-session header, temporarily locks transcript/composer interactions during cold snapshot replay, and ignores stale snapshots/focus intents from older activations;
...
- host → chat webview: `feature.multi-session.chatState`, `feature.multi-session.snapshot`, `feature.multi-session.delta`, guarded one-shot `feature.multi-session.focusInput` for New Chat activations;
