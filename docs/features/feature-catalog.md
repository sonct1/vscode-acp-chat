...
| Rich chat composer and transcript                      | Supports streamed Markdown conversations, code blocks, thought/tool blocks, images, chips, copy/reuse actions, and stop controls.              | Webview input, message list, `sendMessage`, `stop`.                                                   | [`src/views/webview/component/input-panel.ts`](../../src/views/webview/component/input-panel.ts), [`src/views/webview/component/message-list.ts`](../../src/views/webview/component/message-list.ts), [`src/views/webview/block/`](../../src/views/webview/block/)                                                 |
...
Agents are local CLI processes launched through ACP over stdio. Built-in agents are defined in source and merged with `vscode-acp-chat.customAgents`; custom agents with the same `id` replace built-ins, while new ids extend the list. Availability is checked by command lookup before agents appear in the selector.

Built-in agent ids currently include:

`opencode`, `claude-code`, `codex`, `gemini`, `goose`, `amp`, `aider`, `augment`, `kimi`, `mistral-vibe`, `openhands`, `qwen-code`, `kiro`, `cursor`, and `codebuddy`.
...
The session toolbar is capability-driven. If an agent advertises modes, models, generic config options, available commands, or context usage, the webview displays the relevant selector or usage control.
