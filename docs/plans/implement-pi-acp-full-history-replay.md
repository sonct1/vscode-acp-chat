...
| Status     | Draft                                                                                                                                                                                                                              |
| Owner      | TBD                                                                                                                                                                                                                                |
| Scope      | Bundled Pi ACP adapter history loading, Pi JSONL transcript parsing, ACP replay translation, tests, docs, VSIX verification                                                                                                        |
...
### 1. Do not use Pi RPC `get_messages` for history replay

`get_messages` là compacted context cho continuation, không phải full UI transcript.
...
src/features/pi-agent/vendor/pi-acp/src/acp/agent.ts
  # loadSession uses readPiSessionTranscript(stored.sessionFile) for replay
  # proc.getMessages remains fallback only if transcript read fails or has no replayable messages
...
src/features/pi-agent/vendor/pi-acp/test/component/session-list-and-load.test.ts
  # loadSession uses JSONL transcript, not fake getMessages compacted response
...
README.md / docs/features/feature-catalog.md
  # only if implementation changes user-visible Pi history behavior docs
```
...
### Phase 2: Switch `loadSession()` replay source

Update `PiAcpAgent.loadSession()`:

1. Resolve stored session with existing `findStoredSession()`.
2. Restore Pi process as today.
3. Read transcript from `stored.sessionFile`.
4. Replay transcript messages through existing ACP update mapping.
5. Use `proc.getMessages()` only as fallback if JSONL replay fails or returns no replayable messages.

Pseudo-flow:

```ts
let messages = readPiSessionTranscript(stored.sessionFile);
...
- `loadSession()` does not call `proc.getMessages()` when JSONL replay succeeds.
...
5. **Fallback path**
   ...
   If JSONL file missing/unreadable, mocked `getMessages()` compacted fallback is used.
...
3. Load session `019f5e46` or another compacted Pi session from history.
4. Verify early user prompts and assistant replies from before compaction are visible.
5. Verify tool calls still render in order.
6. Continue the loaded session with a new prompt to ensure restored Pi process still works.
7. Run `Developer: Reload Window` after installing the VSIX.
...
- Bundled Pi `session/load` replays from raw JSONL active path.
- Compacted Pi sessions show messages from before the final compaction.
- `proc.getMessages()` is not the primary replay source anymore.
